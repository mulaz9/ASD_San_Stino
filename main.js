...
const getNews = async function () {
  const response = await fetch(
    "https://acd-san-stino.herokuapp.com/api/news-sections?sort[0]=publishedAt:desc&pagination[limit]=3&populate=foto"
  );
...
const getVideos = async function () {
  // Ottenere dati API
  const response = await fetch(
    "https://acd-san-stino.herokuapp.com/api/video-sections?sort[0]=publishedAt:desc&pagination[limit]=3&populate=video"
  );
  const newsJson = await response.json();
  const data = newsJson.data;
  console.log(data);
